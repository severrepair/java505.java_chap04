...
//        메소드:함수와 동일한 개념, 클래스의 멤버로 포함되어 있으면 메소드라고 함
//        사용자가 원하는 특정 결과를 얻기 위한 연산 식을 하나의 이름으로 묶어둔 것
//        사용자가 원하는 결과를 얻기 위한 소스코드를 한번 입력할 때는 문제가 없지만 여러번 사용할 경우 소스코드에 중복이 발생함
//        매개변수 : 해당 메소드 내의 연산 시 필요한 데이터를 외부에서 전달하는 변수
//        반환값 : 해당 메서드의 연산 후 그 결과값을 메소드를 호출한 위치로 다시 되돌려주는 것, 키워드 return 사용

//        메소드 사용법
/*        선언 :
        접근 제한자 반환타입 메소드명(매개변수1, 매개변수2,....){
            실행할 소스코드
    }*/

/*        사용(호출):
        매소드명(매개변수1, 매개변수2,....);
        변수명 = 매소드명(매개변수1, 매개변수2,....);*/
        
/*        매소드의 4가지 형태
        1. 매개변수와 반환값이 모두 없는 형태
          연산에 필요한 변수를 메소드 내부에서 선언하고, 사용하고, 삭제함
          연산 후 그 결과를 메소드 내부에서 표현함
          연산의 결과가 동일함
        2. 매개변수는 존재하고, 반환값은 없는 형태
          연산에 필요한 데이터를 메소드 외부에서 전달받음
          연산후 그 결과를 메소드 내부에서 표현함
          연산의 결과가 입력되는 데이터따라 변경됨
        3. 매개변수는 없고, 반환값은 존재하는 형태
          연산에 필요한 데이터를 메소드 내부에서 선언하고 사용하고 삭제함
          연산후 그 결과를 매소드 외부로 되돌려줌
          연산 결과를 다른 곳에서 활용할 수 있음
          연산의 결과가 동일함
        4. 매개변수와 반환값이 모두 존재하는 형태
          연산에 필요한 데이터를 메소드 외부에서 전달받음
          연산 후 그 결과를 메소드 외부로 되돌려줌
          연산 결과를 다른 곳에서 활용할 수 있음
...
//    return 키워드 사용 시 반환되는 값의 데이터 타입은 해당 메소드의 반환 타입과 일치해야 함
